      ...
      <div className="p-4 bg-orange-50 rounded-3xl shadow-sm border border-orange-100/50">
        <div className="flex justify-between items-center mb-3">
          ...
          <span className="text-xs bg-orange-200 text-orange-800 px-2 py-0.5 rounded-full font-semibold">
            {Array.isArray(notes) ? notes.length : 0} avisos
          </span>
        </div>

        <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
          {!Array.isArray(notes) ? (
            <p className="text-xs text-gray-500 text-center italic py-4">Cargando notas...</p>
          ) : notes.length === 0 ? (
            <p className="text-xs text-gray-500 text-center italic py-4">No hay notas hoy. ¡Todo está tranquilo!</p>
          ) : (
            notes.map((note) => (
              <div key={note.id} className="p-3 bg-white rounded-2xl border border-slate-100 shadow-sm text-slate-800">
